...
class iLQR_Calculator():
    ...
    '''
    def __init__(self, sys, n_x, n_u, dt, goal):
        """
        Class to calculate an optimal trajectory with an iterative
        linear quadratic regulator (iLQR). This implementation uses sympy.
    ...
    def rollout(self, u_trj):
        ...
        for u in u_trj:
            i = i+1
            x = self.discrete_dynamics(self.sys, x, u, self.dt)
            x_trj[i, :] = x
        return x_trj
    ...
    def cost_trj(self, x_trj, u_trj):
        ...
        N = x_trj.shape[0]
        x_trj_cost = x_trj
        # x_trj_cost[:, 1] = (x_trj[:, 1] + np.pi) % (2 * np.pi) - np.pi
        for i in range(len(x_trj)-1):
            ln = ln + self.stage_cost(x_trj_cost[i, :], u_trj[i, :]) / N
        lf = self.final_cost(x_trj_cost[i+1, :], self.goal)
        total = ln + lf
        return total
    ...
    def init_derivatives(self):
        ...
        f = self.discrete_dynamics(self.sys, x, u, self.dt)
        print(x)
    ...
    def forward_pass(self, x_trj, u_trj, k_trj, K_trj):
        ...
        for n in range(u_trj.shape[0]):
            u_trj_new[n, :] = u_trj[n] + k_trj[n] \
                              + np.matmul(K_trj[n],
                                          ((x_trj_new[n] - x_trj[n])))
            x_trj_new[n+1, :] = self.discrete_dynamics(self.sys, x_trj_new[n],
                                                       u_trj_new[n], self.dt)
    ...
    def run_ilqr(self, N=50, init_u_trj=None, init_x_trj=None, shift=False,
        ...
        if init_u_trj is not None:
            ...
            u_trj = np.random.randn(N-1, self.n_u)*0.01
            t = np.linspace(0, N, N - 1) * self.dt
            # u_trj = 3 * np.sin(2 * np.pi * t * 0.2)[:, np.newaxis]
            # u_trj = np.ones(N-1)[:, np.newaxis] * 2
        ...
        if init_x_trj is not None:
            ...
            if shift:
                ...
                x_trj = np.append(x_trj,
                                  [self.discrete_dynamics(self.sys, x_trj[-1],
                                   np.array(u_trj[-1]), self.dt)], axis=0)
        else:
            x_trj = self.rollout(u_trj)
            # x_trj[:, 1] = (x_trj[:, 1] + np.pi) % (2 * np.pi) - np.pi
        ...
        for it in range(max_iter):
            ...
            # x_trj_new[:, 1] = (x_trj_new[:, 1] + np.pi) % (2 * np.pi) - np.pi
