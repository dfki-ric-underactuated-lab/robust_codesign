...
def getEllipseContour(indexes,rho, M,x0, nSamples = 1000):
    """
    ...
    """
    c = sample_from_ellipsoid(indexes,M,rho,r_i = 0.99) +x0
    for i in range(nSamples-1):
        xBar = sample_from_ellipsoid(indexes,M,rho,r_i = 0.99)
...
def ellipseVolume_convexHull(rho, M, x0):
    ...
    x0 : np.array
        center of the ellipse
    '''
    c1 = getEllipseContour((0,1),rho,M, [x0[0],x0[1]], nSamples=1000)
    c2 = getEllipseContour((2,3),rho,M, [x0[0],x0[1]], nSamples=1000)
    #c = getEllipseContour((0,1),rho,M, [x0[0],x0[1]], nSamples=50).T
    #for idxs in [(0,2), (0,3), (1,2), (1,3), (2,3)]:
    #    c = np.vstack((c,getEllipseContour(idxs,rho,M, [x0[idxs[0]],x0[idxs[1]]], nSamples=50).T)) # get the contour of the ellipses
    points = np.vstack((c1[:,0], c1[:,1], c2[:,0], c2[:,1])).T #c.T 
