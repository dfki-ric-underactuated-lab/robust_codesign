...
def prepare_empty(dt=0.001, tf=20):
    n = int(tf/dt)+1

    # create 6 empty numpy array, where desired data can be stored
    des_time_list = np.linspace(0, tf, n)
    des_cart_pos_list = np.zeros(n)
    des_pend_pos_list = np.zeros(n)
    des_cart_vel_list = np.zeros(n)
    des_pend_vel_list = np.zeros(n)
    des_force_list = np.zeros(n)

    # create 6 empty numpy array, where measured data can be stored
    mea_time_list = np.zeros(n)
    mea_cart_pos_list = np.zeros(n)
    mea_pend_pos_list = np.zeros(n)
    mea_cart_vel_list = np.zeros(n)
    mea_pend_vel_list = np.zeros(n)
    mea_force_list = np.zeros(n)
...
def prepare_trajectory(csv_path):
    ...
    des_cart_pos_list = trajectory.T[1].T  # desired cart position in m
    des_pend_pos_list = trajectory.T[2].T  # desired pendulum position in radians
    des_cart_vel_list = trajectory.T[3].T  # desired cart velocity in m/s
    des_pend_vel_list = trajectory.T[4].T  # desired pendulum velocity in radians/s
    des_force_list = trajectory.T[5].T  # desired force in N
