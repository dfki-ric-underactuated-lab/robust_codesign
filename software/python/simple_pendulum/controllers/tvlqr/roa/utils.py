...
def getEllipseContour(S,rho,xg, nSamples = 1000):
    """
...
def funnelVolume(funnel_path, traj_path):
    """
    Returns the volume of a given funnel.

    Parameters
    ----------
    S : np.array
        Matrix S that define the ellipses of a funnel
    rho : np.array
        rho value that define the ellipses of a funnel

    Returns
    -------
    V_i : float
        value proportional to the volume of the given funnel
    """
    ...
    for i in range(len(time)):
        ...
        V = V + np.sqrt((rho_i**n)/np.linalg.det(S_i/rho_i))*dt
    return V
...
def funnelVolume_convexHull(funnel_path, traj_path):
    ...
    for i in range(len(time)-1):
        ...
        c_next = getEllipseContour(S_iplus1,rho_iplus1, np.array(x0).T[i+1], nSamples=200) # get the contour of the next ellipse
        points = np.vstack((c_next[:,0], c_next[:,1], np.linspace(time[i], time[i+1], len(c_next[:,0])))).T
